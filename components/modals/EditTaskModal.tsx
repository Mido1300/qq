...
export default function EditTaskModal({ isOpen, onClose, task }: EditTaskModalProps) {
  ...
  const [subtasks, setSubtasks] = useState<string[]>(task.subtasks.length > 0 ? [...task.subtasks] : ['']);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateTask(task.id, {
      ...
      assignedTo: parseInt(formData.assignedTo) || task.assignedTo,
      subtasks: filteredSubtasks,
      notes: formData.notes,
    });
    
    // Show notification
    showNotification('Task Updated', `"${formData.title}" has been updated`);
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} title="Edit Task">
