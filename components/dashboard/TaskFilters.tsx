...
export default function TaskFilters() {
  ...
  const handleFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    // Handle date filter: show modal for 'custom', reset dateRange for others
    if (id === 'date-filter') {
      if (value === 'custom') {
      ...
      setFilters({
        ...filters,
        date: value,
        dateRange: { start: null, end: null } // Reset dateRange for non-custom filters
      });
      return;
    }

    // Handle other filters (category, priority, status)
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center">
          <button
            ...
            className={`py-2 px-3 rounded-l-lg shadow ${
              view === 'list'
          ...
          <button
            ...
            className={`py-2 px-3 rounded-r-lg shadow border-l ${
              view === 'graph'
                ? 'bg-primary text-white'
                : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300'
            }`}
